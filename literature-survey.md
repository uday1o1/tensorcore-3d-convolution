...
Read in full text directly, not from abstract or memory. Baseline model and the primary target for the convolution kernel substitution. MedNeXt is a ConvNeXt-style architecture built on large-kernel three-dimensional convolutions. Its own text states directly: "The computational requirements of indefinitely scaling kernel sizes in 3D networks quickly becomes prohibitive," which is why the authors introduce compound scaling, balancing kernel size against network width and depth rather than enlarging kernels alone. This confirms, in the paper's own words, that convolution compute cost is a real, acknowledged constraint in this exact architecture, making it a directly motivated target for a memory-efficient convolution kernel rather than an arbitrary choice.
...
Second baseline. The preprint states directly: "Medical Image Segmentation is currently dominated by deep convolutional neural networks (CNNs)," and describes nnU-Net as "a robust and self-adapting framework on the basis of 2D and 3D vanilla U-Nets," evaluated directly on the Medical Segmentation Decathlon, the same benchmark this project uses. Still the field's standard reference framework for medical segmentation and under active maintenance, though the original paper predates the course's recency window; it is included as a required, currently-maintained baseline, not as a recent SOTA claim.
