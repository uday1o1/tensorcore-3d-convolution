...
1. Check Im2win's windowing scheme against MedNeXt's actual kernel shapes. Im2win has only been tested on 2D kernel shapes (its 2026 evaluation covers sizes up to 11 in a single spatial dimension); MedNeXt uses 3D kernels at sizes 3 and 5 per axis. The open question is dimensionality and shape, not size, since the windowing scheme has not previously been exercised on a genuinely three-dimensional kernel.
