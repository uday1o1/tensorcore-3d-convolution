...
1. A reproduction of Im2win's published 2D comparison with full accounting, establishing that the kernel-level advantage is real (1.56 times) but that the end-to-end advantage under honest accounting falls to roughly parity (1.04 to 1.17 times) on the authors' own benchmark shapes and GPU class.
2. An arithmetic-intensity account of what decides whether a windowed method wins on a given layer, yielding the counterintuitive prediction that three-dimensional convolution is more favorable to windowed methods than two-dimensional convolution, because reduction depth grows with the added axis and amortizes the materialization cost.
3. The first extension of the windowing scheme to three dimensions, used to test that prediction directly, with the two-dimensional configuration of the same network as the control arm.
4. An empirical characterization of where windowed convolution does and does not pay off across the layer shapes a real volumetric segmentation workload actually contains, including a measured accounting of where GPU time goes in the standard stack.
