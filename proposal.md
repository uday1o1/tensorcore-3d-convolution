...
**Why this gap is worth closing, not just unclaimed.** MedNeXt's own text states that "the computational requirements of indefinitely scaling kernel sizes in 3D networks quickly becomes prohibitive," which is why its authors introduce compound scaling instead of simply enlarging kernels. This confirms, in the target architecture's own words, that the operation Im2win optimizes, convolution compute cost, is a real and currently unresolved constraint, not a problem this project is inventing to have something to solve. Three further recent papers on efficient 3D medical segmentation were reviewed (`literature-survey.md`, sources 4 through 6); each pursues efficiency through architecture or representation changes, not the convolution kernel, which keeps this project's specific angle open.

**A second, architectural gap.** Im2win's published kernels assume standard dense convolution, a single reduction over `filter_channels x filter_height x filter_width`. MedNeXt's own code shows its primary spatial convolution, `conv1` in every block, is declared with `groups = in_channels`, making it depthwise, not dense; the two are not interchangeable at the kernel level. Dense convolution is what makes Tensor Core utilization possible in the first place, since Tensor Cores operate on the large, regular matrix multiplications that dense reduction produces and gain little from the small per-channel multiplies depthwise convolution performs. Extending Im2win's windowing to the depthwise case directly would require original CUDA kernel design with no existing reference implementation, and depthwise convolution does not present the large dense matmul shape Tensor Cores accelerate, so the expected efficiency gain there is smaller for substantially higher implementation risk. This project therefore converts `conv1` from depthwise to dense convolution as part of its method, then applies the 3D Im2win kernel to the dense layer.
...
1. nnU-Net, the field's standard self-configuring 3D segmentation framework, run on the chosen task using its own standard setup.
2. MedNeXt, the architecture this project modifies, run on the same task using its standard cuDNN-backed 3D convolution.
...
**How the extension actually works.** Im2win's current implementation processes a 2D input by extracting overlapping rectangular windows so that memory reads stay contiguous, avoiding the memory blow-up of the standard im2col approach. Extending this to 3D means generalizing that windowing from two spatial axes to three: instead of a 2D window sliding over height and width, a 3D window must slide over height, width, and depth simultaneously, which changes both the memory layout the kernel expects and the indexing arithmetic inside it. Windowed convolution's memory savings come from bounded overlap redundancy, and that redundancy compounds multiplicatively per added spatial axis: a 2D kernel of size 5 rereads each element up to 25 times, while the 3D equivalent rereads it up to 125 times. At MedNeXt's kernel size of 3, this redundancy (27x) is expected to stay within the technique's viable range; at kernel size 5, the same analysis puts redundancy at a level where the efficiency advantage over cuDNN is not guaranteed, so kernel size 5 falls back to standard cuDNN 3D convolution if the 3D windowed kernel shows no measured speedup at that size.

Im2win's public repository contains only forward-pass CUDA kernels; no backward pass exists to compute gradients with respect to input or weights. Since MedNeXt is trained end to end, this project implements the corresponding backward kernels and wraps both directions in a custom `torch.autograd.Function`, following PyTorch's documented pattern for custom CUDA operators with manual gradients. This is additional, unavoidable implementation work beyond porting the forward kernel to 3D, and is budgeted into the implementation plan below.

MedNeXt's `conv1` layer, the layer this project targets, is depthwise (`groups = in_channels`) in the original architecture. Before the 3D Im2win kernel can be substituted in, this layer is changed to standard dense convolution, matching the reduction shape Im2win's kernels assume. This dense variant becomes the actual baseline `MedNeXt` is compared against for the efficiency claim, so that the ablation changes exactly one factor, the convolution kernel implementation, and not simultaneously the depthwise-to-dense architecture change.

The resulting kernel is wrapped as a PyTorch CUDA extension and substituted for the now-dense `conv1` layers.
...
1. Confirm `conv1` is MedNeXt's compute bottleneck on the Liver task's patch size, via layer-wise FLOP analysis, before committing further implementation effort to accelerating it.
2. Generalize Im2win's windowing computation from two spatial axes to three, targeting dense (non-depthwise) convolution, and implement it as a forward CUDA kernel.
...
4. Validate the extension in isolation, on synthetic 3D convolution shapes, against standard cuDNN 3D convolution for correctness and speed.
5. Change MedNeXt's `conv1` from depthwise to dense convolution, producing the dense-MedNeXt baseline the efficiency claim is measured against.
6. Train nnU-Net and dense-MedNeXt (standard cuDNN 3D convolution) as baselines on the Liver task.
7. Substitute the 3D Im2win kernel into dense-MedNeXt's `conv1` layers and retrain under the identical procedure.
8. Compare all configurations on the metrics defined in Section 1.
