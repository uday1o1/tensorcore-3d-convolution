...
**Named limitation 2.** The reproduction result rests on a transform cost estimated two ways, not on the authors' own transform kernel instrumented in place. A deliberately simple proxy that re-reads the input gave 1.04; an analytic model of a fused transform reading it once gave 1.17. The proxy achieved 835 GB/s, roughly 89 percent of the device's theoretical peak, so the bandwidth constant is sound, and materializing the windowed buffer is intrinsic to the design rather than avoidable by better engineering. Even so, the result must be reported as a bracketed range with both methods stated, not as a single measured figure attributed to the authors' implementation. Closing this bracket by instrumenting the real transform is early work in the implementation plan.

**Named limitation 3.** The central prediction is derived from an analytic intensity model whose one validation attempt against a measured point was imperfect (predicting 1.17 where the proxy measured 1.04, for the reason given above). The model is therefore a hypothesis generator, not evidence. Its per-layer predictions must be confirmed by direct benchmarking on each shape before any of them are reported as findings.
